...
const App = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const firstResponse = await fetch(`http://ec2-18-189-150-72.us-east-2.compute.amazonaws.com:10000/calendar/findbyusernotes/${userData.id}/`, {
          method: "GET",
          headers: {
            'Content-Type': 'application/json',
  ...
  return (
    <div>
      ...
      <div className="flex-1 border p-2">
        <p className="text-center">Health Issue</p>
      </div>
      <div className="flex-1 border p-2">
      ...
      <div ref={ref} className="flex-1 bg-gradient-to-br from-white border-blue-200 p-6 rounded-lg shadow-2xl transform transition-transform duration-300">
        ...
        {isNewEntry || isModifyEntry ?(
          <div className='border border-black p-6'>
            ...
            <input type="text" placeholder="Health Issue" className="mb-2 w-full p-2 rounded" value={name} onChange={handleNameChange} />
            <input type="text" placeholder="yyyy-mm-dd" className="mb-2 w-full p-2 rounded" value={date} onChange={handledate} />
            <input type="text" placeholder="State" className="mb-2 w-full p-2 rounded" value={entry} onChange={handleentryChange} />
